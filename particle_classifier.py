...
def extract_features(events):
    E_l, bx_l, by_l, bz_l, x_l, y_l, z_l = [], [], [], [], [], [], []
    for ev in events:
        E, bx, by, bz, x, y, z = sanitize_event(ev)
        if len(E):
            E_l.append(E); bx_l.append(bx); by_l.append(by); bz_l.append(bz)
            x_l.append(x); y_l.append(y); z_l.append(z)
    cat = lambda l: np.concatenate(l) if l else np.array([])
    X = np.column_stack([cat(E_l), cat(bx_l), cat(by_l), cat(bz_l),
                         cat(x_l), cat(y_l), cat(z_l)])
