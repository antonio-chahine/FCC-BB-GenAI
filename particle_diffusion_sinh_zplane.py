...
# asinh scale for beta_x and beta_y — compress sharp peak, expand tails
ASINH_SCALE_XY = 0.01  # tune: smaller = more compression around peak

# Z-plane decomposition: two interaction planes in nm
Z_PLANES = np.array([-32400000.0, 32400000.0], dtype=np.float32)  # +-32.4 mm in nm
Z_PLANE_THRESH = 5000000.0  # 5 mm in nm — particles within this of a plane get assigned to it
...
class CFG:
    ...
    n_zplane: int = 2
    lambda_zplane: float = 0.3  # weight for z-plane cross-entropy loss
...
class MCPDataset(Dataset):
    ...
    def __init__(self, path, max_particles=64, min_particles=1, keep_fraction=1.0):
        ...
        for ev in raw:
            ...
            # Z-plane decomposition: assign each particle to nearest plane
            z = pos[:, 2]
            dist0 = np.abs(z - Z_PLANES[0])
            dist1 = np.abs(z - Z_PLANES[1])
            zplane_idx = np.where(dist0 <= dist1, 0, 1).astype(np.int64)
            # residual z relative to assigned plane (in nm)
            z_residual = z - Z_PLANES[zplane_idx]
            pos_with_residual = np.concatenate([pos[:, :2], z_residual[:, None]], axis=1)

            cont = np.concatenate(
                [logE[:, None], u_transformed, pos_with_residual],
                axis=1
            ).astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(x0), torch.from_numpy(pdg0), torch.from_numpy(zplane0), torch.from_numpy(mask)
...
class ParticleDenoiser(nn.Module):
    def __init__(self, d_model=128, nhead=4, num_layers=3, dropout=0.1, n_pdg=2, n_zplane=2):
        super().__init__()
        self.d_model = d_model
    ...
    def forward(self, x_t, t, pdg_t, zplane_t, mask):
        ...
        zplane_logits = self.zplane_head(h)  # (B,K,n_zplane)
...
class DDPM:
    ...
    def p_sample(self, x_t, t, pdg_t, zplane_t, mask):
        ...
        Returns (x_prev, pdg_logits, zplane_logits) — logits come for free from the
    ...
    def sample(self, mask, pdg_init, zplane_init):
        ...
        for ti in reversed(range(self.T)):
            ...
            x, logits, zplane_logits = self.p_sample(x, t, pdg, zplane, mask)
            ...
            # Update z-plane
            zplane_probs = torch.softmax(zplane_logits, dim=-1)
            zplane_samp  = torch.multinomial(zplane_probs.view(-1, zplane_probs.size(-1)), 1).view(B, K)
            zplane = torch.where(mask, zplane_samp, zplane)
...
def _decode_batch(x_norm_batch, pdg_idx_batch, zplane_idx_batch, Ks, meta):
    ...
    for i, K in enumerate(Ks):
        ...
        zplane_i  = zplane_idx_batch[i, :K]   # (K,)

        cont = x_i * std + mean
        logE = cont[:, 0]
        u_transformed = cont[:, 1:4]
        pos_xy     = cont[:, 4:6]   # x, y
        ...
        # reconstruct z from plane index + residual
        z_plane_vals = Z_PLANES[zplane_i]  # (K,)
        z = z_plane_vals + z_residual      # (K,)

        E    = np.exp(logE)
        beta = beta_squash_np(u)
        pdg  = np.array([idx_to_pdg[int(j)] for j in pdg_i], dtype=np.int64)
        pos  = np.concatenate([pos_xy, z[:, None]], axis=1)
...
def sample_batch(meta: dict, ddpm: DDPM, device: str, batch_size: int):
    ...
    # Initialise PDG and z-plane uniformly
    ...
    zplane_idx_np  = zplane_idx.cpu().numpy()  # (B, Kmax)

    return _decode_batch(x_norm_np, pdg_idx_np, zplane_idx_np, Ks, meta)
...
Examples:
  ...
  Evaluate: python particle_diffusion.py evaluate --real_path mc_gen1.npy --gen_path results/generated_events.npy
    ...
    if args.mode == 'train':
        ...
        from combined_eval_new import evaluate
        evaluate(args)
    else:
        parser.print_help()
