...
def train(args):
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for x0, pdg0, mask in pbar:
            ...
            # SNR weighting: higher weight at low-t (fine detail) steps
            snr    = ddpm.acp[t] / (1.0 - ddpm.acp[t])          # (B,)
            weight = (snr / (snr + 1.0)).view(-1, 1)             # (B,1), in (0,1)
            diff_loss = ((mse * mask) * weight).sum() / mask.sum().clamp(min=1)
        ...
        with torch.no_grad():
            ...
            for x0, pdg0, mask in pbarv:
                ...
                mse       = (eps_hat - noise).pow(2).sum(dim=-1)
                snr    = ddpm.acp[t] / (1.0 - ddpm.acp[t])
                weight = (snr / (snr + 1.0)).view(-1, 1)
                diff_loss = ((mse * mask) * weight).sum() / mask.sum().clamp(min=1)
                pdg_loss  = F.cross_entropy(pdg_logits[mask], pdg0[mask])
                c_loss    = charge_balance_loss(pdg_logits, mask)
                loss      = diff_loss + cfg.lambda_pdg * pdg_loss + cfg.lambda_charge * c_loss
