...
for ev in events:
    E_signed = ev[:, 0]
    n_minus = np.sum(E_signed >= 0)   # e-  (PDG 11)
    n_plus  = np.sum(E_signed <  0)   # e+  (PDG -11)
    charge_diff.append(n_minus - n_plus)
    total_mult.append(n_minus + n_plus)
...
plt.figure(figsize=(7, 5))
plt.hist(charge_diff, bins=bins, rwidth=1.0, edgecolor="black")
...
plt.xlabel(r"$N_{e^-} - N_{e^+}$")
plt.ylabel("Number of events")
plt.title("Charge Asymmetry Per Event — Real Data")
plt.tight_layout()
plt.savefig("charge_asymmetry_real.png")
plt.show()