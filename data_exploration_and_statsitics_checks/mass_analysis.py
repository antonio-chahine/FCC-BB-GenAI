...
C_GEN  = "#D6604D"
C_LINE = "#1a1a1a"
C_MREF = "#2ca02c"   # green for m_e reference
...
# ── Helper: extract mass array from one event set ─────────────────────────────
def extract_masses(events, label=""):
    """
    Expects each event to be (N, >=4): [E_signed, bx, by, bz, ...]
    Returns (m array, stats dict).
    """
    ...
    if label:
        print(f"\n=== {label} ===")
        print(f"  Events: {stats['n_events']:,}  |  Particles: {tot:,}")
        print(f"  Neg m² count: {neg_m2}")
        print(f"  mean  = {stats['mean']:.6f} GeV")
        print(f"  median= {stats['median']:.6f} GeV")
        print(f"  peak  = {stats['peak']:.6f} GeV   (m_e = {m_e:.6f})")
        print(f"  std   = {stats['std']:.6f} GeV")
        print(f"  rel err std = {stats['rel_err_std']:.4f}")

    return all_m, all_m2, all_beta2, stats


sim_m, sim_m2, sim_b2, sim_s = extract_masses(simulated_events, "Simulated")
gen_m, gen_m2, gen_b2, gen_s = extract_masses(generated_events, "Generated")


# ── Figure 1: Full distribution (log y) side-by-side with stats panel ─────────
def stats_text(s, color):
    return (
        f"N events : {s['n_events']:,}\n"
        f"N particles : {s['n_particles']:,}\n"
        f"Peak  : {s['peak']:.5f} GeV\n"
        f"Mean  : {s['mean']:.5f} GeV\n"
        f"Median: {s['median']:.5f} GeV\n"
        f"Std   : {s['std']:.5f} GeV\n"
        f"Rel. err σ: {s['rel_err_std']:.4f}"
    )
...
# reference lines
for label_txt, val, col, ls in [
        (r"$m_e$ = 0.511 MeV", m_e, C_MREF, "--"),
]:
    ax.axvline(val, color=col, linestyle=ls, linewidth=1.6,
               label=label_txt)

# annotate mean/peak on plot
...
# stats panel (text only)
ax_info.set_axis_off()
ax_info.text(0.04, 0.97, stats_text(sim_s, C_SIM),
             transform=ax_info.transAxes, va="top", ha="left",
             fontsize=10.5, color=C_SIM,
             bbox=dict(boxstyle="round,pad=0.5", facecolor="#EBF3FB",
                       edgecolor=C_SIM, alpha=0.9))
ax_info.text(0.04, 0.50, stats_text(gen_s, C_GEN),
             transform=ax_info.transAxes, va="top", ha="left",
             fontsize=10.5, color=C_GEN,
             bbox=dict(boxstyle="round,pad=0.5", facecolor="#FDECEA",
                       edgecolor=C_GEN, alpha=0.9))

savefig(fig, "mass_distribution.png")
...
ax2.hist(sim_m, bins=bins_zoom, density=True, alpha=0.55,
         color=C_SIM, label="Simulated")
ax2.hist(gen_m, bins=bins_zoom, density=True, histtype="step",
         linewidth=2.0, color=C_GEN, label="Generated")
