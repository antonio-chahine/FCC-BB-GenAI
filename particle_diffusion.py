...
class CFG:
    ...
    outdir: str = "mc_gen1_model_skip_2"
...
class ParticleDenoiser(nn.Module):
    def __init__(self, vocab_size, d_model=128, nhead=4, num_layers=3, dropout=0.1):
        ...
        self.mom_proj = nn.Linear(3, d_model)
    ...
    def forward(self, x_t, t, pdg_id, mask):
        ...
        t_emb   = self.time_emb(t).unsqueeze(1).expand(B, K, self.d_model)
...
def sample_one_event(meta: dict, ddpm: DDPM, device: str):
    ...
    # --- sample number of e± pairs ---
    multiplicities = np.asarray(meta["multiplicities"], dtype=np.int64)
    K = int(np.random.choice(multiplicities))   # this is total charged particles
    K = K - (K % 2)                             # force even
    K = max(2, min(K, meta["max_particles"]))
    ...
    # optional: shuffle ordering inside event
...
def two_panel_dist(real, gen, outpath, xlabel, title, species_name, bins=80, ratio_min_count=10):
    ...
    rng = peak_centered_range(real, gen, bins=600, frac=0.90)
    if rng is None:
