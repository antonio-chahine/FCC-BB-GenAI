...
def transform_betaxy(b: np.ndarray, c: float = 1.5) -> np.ndarray:
    """
    Map β_x, β_y ∈ (-1,1) → unconstrained v ∈ ℝ.
    atanh lifts to ℝ, then sinh-arcsinh with c>1 stretches the
    near-zero region so the double-peak is visible after normalisation.
    """
    u = np.arctanh(np.clip(b, -1 + 1e-6, 1 - 1e-6))
    return np.sinh(c * np.arcsinh(u)).astype(np.float32)


def inv_transform_betaxy(v: np.ndarray, c: float = 1.5) -> np.ndarray:
    """Inverse of transform_betaxy. Output is guaranteed in (-1,1)."""
    u = np.sinh(np.arcsinh(v) / c)
    return np.tanh(u).astype(np.float32)


def transform_betaz(b: np.ndarray, c: float = 0.5) -> np.ndarray:
    """
    Map β_z ∈ (-1,1) → unconstrained v ∈ ℝ.
    c=0.5 compresses the near-±1 beam core to keep std O(1-3).
    """
    u = np.arctanh(np.clip(b, -1 + 1e-6, 1 - 1e-6))
    return np.sinh(c * np.arcsinh(u)).astype(np.float32)


def inv_transform_betaz(v: np.ndarray, c: float = 0.5) -> np.ndarray:
    """Inverse of transform_betaz. Output is guaranteed in (-1,1)."""
    u = np.sinh(np.arcsinh(v) / c)
    return np.tanh(u).astype(np.float32)


def transform_beta_np(beta: np.ndarray) -> np.ndarray:
    """Transform all three beta components. beta shape: (N, 3)."""
    vxy = transform_betaxy(beta[:, :2])   # (N, 2)
    vz  = transform_betaz(beta[:, 2:3])   # (N, 1)
    return np.concatenate([vxy, vz], axis=1)  # (N, 3)


def inv_transform_beta_np(v: np.ndarray) -> np.ndarray:
    """Inverse transform all three beta components. v shape: (N, 3)."""
    bxy = inv_transform_betaxy(v[:, :2])  # (N, 2)
    bz  = inv_transform_betaz(v[:, 2:3]) # (N, 1)
    return np.concatenate([bxy, bz], axis=1)  # (N, 3)


# Keep old functions so evaluate / sanitize_event still work unchanged
...
# Physical simulation box boundaries from Guinea-Pig (in nm):
#   x, y: ±1e7 nm,  z: ±3.24e7 nm
# Using these as tanh scales means:
#   - forward transform never clips (all training data is within bounds)
#   - inverse is unconditionally bounded — no overflow ever possible
POS_SCALE = np.array([1e7, 1e7, 3.24e7], dtype=np.float32)

def transform_pos(p: np.ndarray) -> np.ndarray:
    """
    Map positions (nm) → unconstrained u ∈ ℝ via arctanh(p / scale).
    The clip guard never fires on real data — it's purely a numerical
    safeguard for the arctanh singularity at ±1.
    """
    return np.arctanh(
        np.clip(p / POS_SCALE, -1 + 1e-6, 1 - 1e-6)
    ).astype(np.float32)


def inv_transform_pos(u: np.ndarray) -> np.ndarray:
    """
    Inverse: u ∈ ℝ → positions in (-POS_SCALE, POS_SCALE).
    tanh is unconditionally bounded so no overflow is possible
    regardless of what the diffusion model samples.
    """
    return (POS_SCALE * np.tanh(u)).astype(np.float32)
...
class CFG:
    ...
    T: int = 500
    beta_start: float = 1.7193632995385393e-05
    beta_end: float = 0.04298284381843208
    
    d_model = 64
    nhead = 8
    num_layers = 3
    dropout = 0.016806160685986136
    
    batch_size: int = 2
    lr: float = 0.00011678063934492703
    epochs: int = 50
    num_workers: int = 8
    grad_clip: float = 4.361964884633818
    seed: int = 123
    ...
    feat_dim: int = 7
    n_pdg: int = 2
    lambda_pdg: float = 3.095677257838085

    gamma_start: float = 1e-4
    gamma_end: float = 0.05431660502161486
...
class MCPDataset(Dataset):
    ...
    Continuous features:
        [log|E|, v_x, v_y, v_z, x, y, z]
    where v = transform_beta_np(beta) using sinh-arcsinh transforms.
    """
    ...
    def __init__(self, path, max_particles=64, min_particles=1, keep_fraction=1.0):
        ...
        for ev in raw:
            ...
            v = transform_beta_np(beta)   # (N, 3)
            pos_t = transform_pos(pos)    # arctanh squash into (-1,1) using physical bounds
        ...
        print("Feature means:", self.feat_mean)
        print("Feature stds: ", self.feat_std)
        print("(v_x, v_y, v_z stds should be O(1-5), not O(0.001) or O(100))")
...
class DDPM:
    ...
    def sample(self, mask, pdg_init):
        ...
        for ti in reversed(range(self.T)):
            ...
            # FIX: p_sample returns logits — no second forward pass needed
...
def _decode_batch(x_norm_batch, pdg_idx_batch, Ks, meta):
    ...
    for i, K in enumerate(Ks):
        ...
        cont  = x_i * std + mean       # back to transform space
        logE  = cont[:, 0]
        v     = cont[:, 1:4]           # sinh-arcsinh space
        pos_t = cont[:, 4:7]

        E    = np.exp(logE)
        beta = inv_transform_beta_np(v)
        pos  = inv_transform_pos(pos_t)
...
def sample_single(meta: dict, ddpm: DDPM, device: str):
    """
    Generate exactly one event (kept for debugging / reference).
    Calls sample_batch with batch_size=1.
    """
...
def plot_corner_overlay(
    ...
    for d in range(len(keys)):
        ...
        lo = np.quantile(z, q_lo)
        hi = np.quantile(z, q_hi)
        ranges.append((lo, hi))
...
Examples:
  ...
  Sample (batched): python particle_diffusion.py sample --outdir results --n_events 1000 --sample_batch_size 32
