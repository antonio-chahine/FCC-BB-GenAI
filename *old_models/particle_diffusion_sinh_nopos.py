...
# Positions are passed through as raw nm values.
# No arctanh/tanh transform — the dataset's z-score normalisation
# (feat_mean / feat_std) handles the scaling instead.
# This avoids the tanh saturation problem where the model outputs
# large latent values that get clipped to ±POS_SCALE.

def transform_pos(p: np.ndarray) -> np.ndarray:
    """Identity: positions passed through in nm, z-scored by the dataset."""
    return np.asarray(p, dtype=np.float32)


def inv_transform_pos(u: np.ndarray) -> np.ndarray:
    """Identity inverse: values are already in nm after z-score un-normalisation."""
...
class MCPDataset(Dataset):
    ...
    def __init__(self, path, max_particles=64, min_particles=1, keep_fraction=1.0):
        ...
        for ev in raw:
            ...
            pos_t = transform_pos(pos)    # identity — z-score normalisation handles scaling
