...
def transform_beta_np(beta: np.ndarray) -> np.ndarray:
    vxy = transform_betaxy(beta[:, :2])
    vz  = transform_betaz(beta[:, 2:3])
    return np.concatenate([vxy, vz], axis=1)


def inv_transform_beta_np(v: np.ndarray) -> np.ndarray:
    bxy = inv_transform_betaxy(v[:, :2])
    bz  = inv_transform_betaz(v[:, 2:3])
    return np.concatenate([bxy, bz], axis=1)
...
class MCPDataset(Dataset):
    def __init__(self, path, max_particles=64, min_particles=1, keep_fraction=1.0):
        ...
        for ev in raw:
            ...
            v     = transform_beta_np(beta)
            pos_t = transform_pos(pos)
...
def sample_batch(meta: dict, ddpm: DDPM, device: str, batch_size: int):
    ...
    All events are denoised simultaneously — one forward pass per
    timestep regardless of batch size.
    """
    ...
    return _decode_batch(x_norm.cpu().numpy(), pdg_idx.cpu().numpy(), Ks, meta)
...
def plot_corner_overlay(
    real_dict, gen_dict, keys, labels, outpath,
    title="", max_points=40000, q_lo=0.01, q_hi=0.99, seed=123, bins=40,
):
...
def sanitize_event(ev, me=0.000511):
    ...
    return (empty.astype(np.int64), empty, empty, empty, empty, empty, empty,
