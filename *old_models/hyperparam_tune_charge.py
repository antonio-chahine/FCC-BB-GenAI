...
from particle_diffusion_charge_loss_and_learning import (
...
def objective(trial, data_path, base_outdir, device):
    ...
    T          = trial.suggest_categorical("T",          [200, 500, 1000])
    ...
    lambda_charge = trial.suggest_float("lambda_charge", 0.01, 1.0, log=True)
    ...
    for epoch in range(cfg.epochs):
        ...
        try:
            ...
            print(f"  Trial {trial.number} OOM during train "
                  f"(batch_size={batch_size}, d_model={d_model}, T={T}) — pruning.")
            raise optuna.exceptions.TrialPruned()
        ...
        try:
            ...
            torch.cuda.empty_cache()
            print(f"  Trial {trial.number} OOM during val — pruning.")
            raise optuna.exceptions.TrialPruned()
