...
class CFG:
    ...
    T: int = 1000
    beta_start: float = 9.718319944817387e-05
    beta_end: float = 0.016661067877731174
    
    d_model = 256
    nhead = 4
    num_layers = 2
    dropout = 0.21397343616689848
    
    batch_size: int = 4
    lr: float = 0.0004064644058668378
    epochs: int = 50
    num_workers: int = 8
    grad_clip: float = 0.6414313355903041
    seed: int = 123
    ...
    feat_dim: int = 7
    n_pdg: int = 2
    lambda_pdg: float = 2.1949509899568933
    lambda_charge: float = 0.011241862095793064  # charge balance loss weight

    gamma_start: float = 0.0008113929572637835
    gamma_end: float = 0.09855114214698701

    n_events = 500
    sample_batch_size: int = 16

    pct_start: float = 0.09715339716144901
    div_factor: float = 32.253042671791206
...
def train(args):
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for x0, pdg0, mask in pbar:
            ...
            # Charge balance loss: expected N(e-) - N(e+) should match ground truth
            # probs[:,0] = P(electron), probs[:,1] = P(positron)
            probs_pdg  = torch.softmax(pdg_logits, dim=-1)            # (B, K, 2)
            charge_exp = (probs_pdg[:, :, 0] - probs_pdg[:, :, 1])   # (B, K)
            charge_exp = (charge_exp * mask.float()).sum(dim=1)        # (B,)
            charge_true = ((pdg0 == 0).float() - (pdg0 == 1).float()) * mask.float()
            charge_true = charge_true.sum(dim=1)                       # (B,)
            charge_loss = (charge_exp - charge_true.detach()).pow(2).mean()

            loss = diff_loss + cfg.lambda_pdg * pdg_loss + cfg.lambda_charge * charge_loss
            ...
            pbar.set_postfix(
                ...
                pdg=f"{pdg_loss.item():.4f}",
                chg=f"{charge_loss.item():.4f}",
        ...
        with torch.no_grad():
            ...
            for x0, pdg0, mask in pbarv:
                ...
                probs_pdg  = torch.softmax(pdg_logits, dim=-1)
                charge_exp = (probs_pdg[:, :, 0] - probs_pdg[:, :, 1])
                charge_exp = (charge_exp * mask.float()).sum(dim=1)
                charge_true = ((pdg0 == 0).float() - (pdg0 == 1).float()) * mask.float()
                charge_true = charge_true.sum(dim=1)
                charge_loss = (charge_exp - charge_true).pow(2).mean()
