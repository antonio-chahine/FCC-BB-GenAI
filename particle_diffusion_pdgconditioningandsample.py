...
class ParticleDenoiser(nn.Module):
    def __init__(self, d_model=128, nhead=4, num_layers=3, dropout=0.1, n_pdg=2):
        ...
        self.k_minus_mlp = nn.Sequential(
            nn.Linear(1, d_model),
            nn.SiLU(),
            nn.Linear(d_model, d_model),
        )

        self.k_plus_mlp = nn.Sequential(
            nn.Linear(1, d_model),
            nn.SiLU(),
            nn.Linear(d_model, d_model),
        )
    ...
    def forward(self, x_t, t, pdg_ids, mask):
        B, K, _ = x_t.shape
        ...
        pdg_emb = self.pdg_emb(pdg_ids.clamp(0, self.pdg_emb.num_embeddings - 1))

        h = t_emb + mom_emb + pdg_emb

        # Per-species multiplicity conditioning:
        # tell the model exactly how many e- and e+ are in this event
        K_minus = (pdg_ids == 0).float().sum(dim=1)   # n electrons (B,)
        K_plus  = (pdg_ids == 1).float().sum(dim=1)   # n positrons (B,)

        k_minus_emb = self.k_minus_mlp(
            torch.log(K_minus.clamp(min=1)).unsqueeze(-1)
        ).unsqueeze(1)                                 # (B,1,d)

        k_plus_emb = self.k_plus_mlp(
            torch.log(K_plus.clamp(min=1)).unsqueeze(-1)
        ).unsqueeze(1)                                 # (B,1,d)

        h = h + k_minus_emb + k_plus_emb
...
class DDPM:
    ...
    def p_sample(self, x_t, t, pdg_ids, mask):
        """Single denoising step. PDG is a fixed conditioning input, not predicted."""
        B = x_t.shape[0]

        eps_hat = self.model(x_t, t, pdg_ids, mask)
    ...
    @torch.no_grad()
    def sample(self, pdg_ids, mask):
        """
        Full reverse diffusion. PDG is fixed as conditioning input throughout —
        it is never updated or predicted during sampling.
        """
        B, K = mask.shape
        ...
        for ti in reversed(range(self.T)):
            ...
            x = self.p_sample(x, t, pdg_ids, mask)
...
def train(args):
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for x0, pdg0, mask in pbar:
            ...
            # PDG is fixed conditioning — never corrupted
            eps_hat = model(x_t, t, pdg0, mask)
...
def sample_batch(meta: dict, ddpm: DDPM, device: str, batch_size: int):
    """
    Generate `batch_size` events with balanced PDG conditioning:
      1. Sample total multiplicity K from the empirical distribution.
      2. Assign K//2 electrons (idx=0) and K-K//2 positrons (idx=1).
         PDG is fixed for the entire denoising chain — charge balance
         is guaranteed by construction.
      3. The model sees the true per-species counts via k_minus_mlp /
         k_plus_mlp, so conditioning is consistent with training.
    """
    multiplicities = np.asarray(meta["multiplicities"], dtype=np.int64)
    Kmax = int(meta["max_particles"])

    # ── 1. Draw total multiplicities, force even, split 50/50 ────────────────
    Ks_total = np.random.choice(multiplicities, size=batch_size, replace=True)
    Ks_total = Ks_total - (Ks_total % 2)          # force even
    Ks_total = np.clip(Ks_total, 2, Kmax).astype(np.int64)

    Ks_minus = Ks_total // 2                       # n electrons per event
    Ks_plus  = Ks_total - Ks_minus                 # n positrons per event

    # ── 2. Build mask and fixed PDG tensor ────────────────────────────────────
    # Layout: first K_minus slots = electrons (0), next K_plus slots = positrons (1)
    mask_np  = np.zeros((batch_size, Kmax), dtype=np.bool_)
    pdg_np   = np.zeros((batch_size, Kmax), dtype=np.int64)

    for i, (km, kp) in enumerate(zip(Ks_minus, Ks_plus)):
        K = km + kp
        mask_np[i, :K]   = True
        pdg_np[i, :km]   = 0   # electrons
        pdg_np[i, km:K]  = 1   # positrons
        # shuffle so model doesn't learn order artefacts
        perm = np.random.permutation(K)
        pdg_np[i, :K] = pdg_np[i, :K][perm]

    mask_t = torch.from_numpy(mask_np).to(device)
    pdg_t  = torch.from_numpy(pdg_np).to(device)

    # ── 3. Single denoising pass with fixed PDG ───────────────────────────────
    with torch.no_grad():
        x_norm = ddpm.sample(pdg_t, mask_t)   # (B, Kmax, 7)

    x_norm_np = x_norm.cpu().numpy()
    pdg_np_out = pdg_t.cpu().numpy()

    return _decode_batch(x_norm_np, pdg_np_out, Ks_total, meta)
...
Examples:
  ...
  Evaluate: python particle_diffusion.py evaluate --real_path mc_gen1.npy --gen_path results/generated_events.npy
    ...
    eval_parser.add_argument('--gen_path',        type=str,
                             default=os.path.join(cfg.outdir, "generated_events_2.npy"))
