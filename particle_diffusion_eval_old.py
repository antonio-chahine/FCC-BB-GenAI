...
def _peak_zoom_range(arr, zoom_frac=0.01):
    """Return (lo, hi) centred on the histogram peak, spanning zoom_frac of full width."""
    arr = arr[np.isfinite(arr)]
    if arr.size == 0:
        return None
...
def plot_single_three_zoom(real, gen, outpath, xlabel, title, species_name,
    ...
    """
    Three columns side by side, each with histogram + ratio panel:
      col 0 — full range
      col 1 — slight zoom around peak
      col 2 — tight zoom around peak
    """
...
# ── Energy — delegates to three-zoom ─────────────────────────────────────────
...
def plot_energy_from_mass(real_E, gen_beta, gen_E_sign, outpath, species_name,
                          ...
                          n_real=0, n_gen=0):
    """
    Compare real stored energy vs generated energy recomputed from momentum
    using the electron proper mass:  E = sqrt(|p|^2 + me^2),
    with sign restored from the original E_signed sign.

    gen_beta : array of shape (N, 3) — (betax, betay, betaz) for generated particles
    gen_E_sign : array of shape (N,) — the stored E_signed (used only for its sign)
    """
    # Recompute |p| = |E_stored| * |beta|, then E = sqrt(p^2 + me^2)
    ...
    p_mag     = gen_Eabs * beta_mag                       # |p| = E_stored * |β|
    E_proper  = np.sqrt(p_mag**2 + me**2)                 # E = sqrt(p²+m²)
...
# ── 3-component grouped subplot (px/py/pz or x/y/z) — 3 zoom rows ────────────
def three_panel_three_zoom(real_dict, gen_dict,
                            ...
                            tight_zoom_frac=0.005):
    """
    Layout: 3 columns (one per component) × 3 zoom-rows (full / slight / tight),
    each zoom-row contains a histogram + ratio panel.
    """
    assert len(keys) == 3
...
def _find_closest_positron_opening_angles(electrons_p, positrons_p):
    """
    For each electron momentum vector, find the positron with the smallest
    opening angle and return that angle.
    θ = arccos( p̂_e · p̂_e+ )
    """
    angles = []
    ...
    e_hat = electrons_p / e_norms        # (N_e, 3)
    p_hat = positrons_p / p_norms        # (N_p, 3)

    # dot products matrix (N_e, N_p)
    dots = np.clip(e_hat @ p_hat.T, -1.0, 1.0)
    # opening angles matrix
    angles_mat = np.arccos(dots)         # radians
    # closest positron for each electron
...
def _extract_momenta_from_events(events, pdg_code, is_generated=False):
    """
    Extract (px, py, pz) arrays for all particles with given pdg_code,
    grouped by event.  Returns list of (N_particles, 3) arrays.
    """
    per_event = []
    for ev in events:
        ...
        if ev.ndim != 2:
            continue
        if is_generated:
            # columns: [pdg, E_signed, betax, betay, betaz]
            if ev.shape[1] < 5:
                continue
            pdgs  = ev[:, 0]
            mask  = (pdgs == pdg_code)
            E     = np.abs(ev[mask, 1])
            beta  = ev[mask, 2:5]
            # p = E * beta  (since p = γmβ and E = γm)
            p     = E[:, None] * beta
        else:
            # columns: [E_signed, betax, betay, betaz]  (no pdg column)
            if ev.shape[1] < 4:
                continue
            # For real events we rely on the caller having split by species already.
            # We just use all rows.
            E     = np.abs(ev[:, 0])
            beta  = ev[:, 1:4]
            p     = E[:, None] * beta
        per_event.append(p)
...
def compute_opening_angles_from_events(events):
    """
    Compute per-event closest e-/e+ opening angles from generated raw events.
    Generated event columns: [pdg, E_signed, betax, betay, betaz]
    """
    all_angles = []
    for ev in events:
        ev = np.asarray(ev, dtype=np.float64)
...
def compute_opening_angles_from_species(sp_eminus, sp_eplus):
    """
    Compute opening angles from extract_species output dicts.
    Works for both real and generated data since species are already split.
    Uses px/py/pz and the mult array to reconstruct per-event grouping.
    """
    def iter_event_p(sp):
...
def plot_opening_angles(real_sp_eminus, real_sp_eplus,
                        ...
                        bins=80, ratio_min_count=10):
    """
    Plot closest e+/e- pair opening angle distribution.
    Three fixed zoom levels:
      col 0 — full range 0–180 deg
      col 1 — slight zoom 0–30 deg  (where the bulk of the distribution lives)
      col 2 — tight zoom  0–5 deg   (the sharp peak region)
    """
    ...
    # Fixed physically meaningful zoom ranges (degrees)
    zoom_cols = [
...
# ── Charge asymmetry (from check_charge.py) ───────────────────────────────────
...
def evaluate(args):
    ...
    for sp in species_list:
        ...
        # 3) Beta components — 3 zoom levels each
        ...
        # 4) Momentum grouped — 3 rows (full/slight/tight) × 3 cols (px/py/pz)
        ...
        # 5) Position components — 3 zoom levels each
