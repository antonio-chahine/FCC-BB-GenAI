...
Z_PLANES = np.array([-32400000.0, 32400000.0], dtype=np.float32)  # +-32.4 mm
...
class CFG:
    ...
    T: int = 1000
    cosine_s: float = 0.014114714256302887

    d_model = 256
    nhead = 2
    num_layers = 3
    dropout = 0.1798900293931045
    
    batch_size: int = 32
    lr: float = 0.00010359525611866081
    epochs: int = 50
    num_workers: int = 8
    grad_clip: float = 4.226585469986976
    seed: int = 123
    ...
    feat_dim: int = 7
    n_pdg: int = 2
    lambda_pdg: float = 0.06839757579644731
    lambda_charge: float = 0.009407242380623418
    n_zplane: int = 2
    lambda_zplane: float = 1.3655289041190926

    gamma_start: float = 0.0005950493881684407
    gamma_end: float = 0.45619571692190314

    n_events = 500
    sample_batch_size: int = 16

    pct_start: float = 0.05096881460738059
    div_factor: float = 28.89050306921334
...
class DDPM:
    ...
    def p_sample(self, x_t, t, pdg_t, zplane_t, mask):
        ...
        Returns (x_prev, pdg_logits, zplane_logits).
        """
        B = x_t.shape[0]
...
def _decode_batch(x_norm_batch, pdg_idx_batch, zplane_idx_batch, Ks, meta):
    ...
    for i, K in enumerate(Ks):
        ...
        z_plane_vals = Z_PLANES[zplane_i]
        z = z_plane_vals + z_residual
...
def sample_batch(meta: dict, ddpm: DDPM, device: str, batch_size: int):
    ...
    zplane_idx_np = zplane_idx.cpu().numpy()
