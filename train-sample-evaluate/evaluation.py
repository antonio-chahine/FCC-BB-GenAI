...
GEN_PATH  = "/work/submit/anton100/msci-project/FCC-BB-GenAI/mc_gen1_model_cosine/generated_events.npy"
OUTDIR    = "/work/submit/anton100/msci-project/FCC-BB-GenAI/mc_gen1_model_cosine/plots"
...
def peak_centered_range(x, y, bins=400, frac=0.995, min_width=1e-12):
    """
    Find a plot range centered on the histogram peak (mode), by expanding
    from the max-count bin until cumulative mass reaches `frac`.
    ...
    # get a finite overall range first (avoid degenerate bins)
    zmin = float(np.min(z))
    zmax = float(np.max(z))
    ...
    i0 = int(np.argmax(counts))   # peak bin index
    lo_i = hi_i = i0
    cum = int(counts[i0])

    # expand outward around the peak until we contain `frac` of events
    target = frac * tot
    while cum < target and (lo_i > 0 or hi_i < len(counts) - 1):
        left_count  = counts[lo_i - 1] if lo_i > 0 else -1
        right_count = counts[hi_i + 1] if hi_i < len(counts) - 1 else -1
