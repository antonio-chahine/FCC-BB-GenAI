...
Changes vs base:
  ...
  - No asinh transform: u_x, u_y, y position passed directly (z-score normalisation only)
...
def charge_balance_loss(pdg_logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    Real data has exactly N(e-) == N(e+) per event (sigma=0).
    Penalises the squared expected charge imbalance per event,
    computed differentiably through the PDG softmax probabilities.

    pdg_logits: (B, K, 2)  where index 0 = e-, index 1 = e+
    mask:       (B, K) bool
    Returns scalar loss.
    """
    probs   = torch.softmax(pdg_logits, dim=-1)          # (B, K, 2)
    p_eminus = probs[:, :, 0] * mask                     # (B, K)
    p_eplus  = probs[:, :, 1] * mask                     # (B, K)
    # expected charge imbalance per event
    imbalance = (p_eminus - p_eplus).sum(dim=1)          # (B,)
...
def asinh_transform_xy(u_xy: np.ndarray, scale: float = None) -> np.ndarray:
    """Identity transform -- no asinh applied to beta_x, beta_y."""
    return np.asarray(u_xy, dtype=np.float32)


def asinh_inverse_xy(v_xy: np.ndarray, scale: float = None) -> np.ndarray:
    return np.asarray(v_xy, dtype=np.float32)


def asinh_transform_y(y: np.ndarray, scale: float = None) -> np.ndarray:
    """Identity transform -- no asinh applied to y position."""
    return np.asarray(y, dtype=np.float32)


def asinh_inverse_y(v: np.ndarray, scale: float = None) -> np.ndarray:
...
class CFG:
    ...
    feat_dim: int = 7   # [logE, u_x, u_y, u_z, x, y, z_residual]
    n_pdg: int = 2
    n_zplane: int = 2
...
class MCPDataset(Dataset):
    ...
    Continuous features stored (7 cols):
        [logE, u_x, u_y, u_z, x, y, z_residual]

        No asinh transforms applied -- z-score normalisation only.
    """
    ...
    def __init__(self, path, max_particles=1300, min_particles=1, keep_fraction=1.0):
        ...
        for ev in raw:
            ...
            # u_x, u_y passed directly (no asinh)
            ux_t = u[:, 0:1].astype(np.float32)            # (N,1)
            uy_t = u[:, 1:2].astype(np.float32)            # (N,1)
            ...
            x_pos = pos[:, 0:1]
            y_pos = pos[:, 1:2].astype(np.float32)  # y passed directly (no asinh)
...
def _decode_batch(x_norm_batch, pdg_idx_batch, zplane_idx_batch, Ks, meta):
    ...
    for i, K in enumerate(Ks):
        ...
        cont = x_i * std + mean
        # cols: [logE, u_x, u_y, u_z, x, y, z_residual]
        logE       = cont[:, 0]
        u_x        = cont[:, 1]
        u_y        = cont[:, 2]
        u_z        = cont[:, 3]
        ...
        # squash u_x, u_y to beta_x, beta_y
        u_xy    = np.stack([u_x, u_y, np.zeros_like(u_x)], axis=1)
