...
# cols: 0=E  1=px  2=py  3=pz  4=betax  5=betay  6=betaz
...
def sanitize_event(ev):
    """
    Returns (E, px, py, pz, betax, betay, betaz).
    px/py/pz = E * beta components — physically meaningful momentum coordinates.
    Supports input arrays with 8 columns (col0 ignored) or 7 columns.
    """
    ev = np.asarray(ev)
    if ev.ndim == 2 and ev.shape[1] >= 8:
        Eabs = np.abs(ev[:, 1])
        bx, by, bz = ev[:, 2], ev[:, 3], ev[:, 4]
    elif ev.ndim == 2 and ev.shape[1] >= 7:
        Eabs = np.abs(ev[:, 0])
        bx, by, bz = ev[:, 1], ev[:, 2], ev[:, 3]
    else:
        e = np.array([])
    ...
    py = Eabs * by
    pz = Eabs * bz
    return Eabs, px, py, pz, bx, by, bz

def knn_edges(pos, k):
    """Build kNN graph. pos should be (N, 3) — here px/py/pz."""
    n = len(pos)
...
def event_to_graph(ev_raw, label, k=8):
    E, px, py, pz, bx, by, bz = sanitize_event(ev_raw)
    if len(E) == 0: return None
    # Feature matrix: [E, px, py, pz, betax, betay, betaz]
    X = np.column_stack([E, px, py, pz, bx, by, bz])
    X = X[np.all(np.isfinite(X), axis=1)]
    if len(X) == 0: return None
    return Data(
        x=torch.tensor(X, dtype=torch.float),
        edge_index=knn_edges(X[:, 1:4], k),   # kNN in px/py/pz space
...
class GNN(nn.Module):
    def __init__(self, in_ch=7, h=32):
        ...
        def block(a, b):
            mlp = nn.Sequential(
                nn.Linear(2 * a, b), nn.BatchNorm1d(b), nn.ReLU(),
                nn.Linear(b, b),     nn.BatchNorm1d(b), nn.ReLU()
            )
            return EdgeConv(mlp, aggr='mean')
        self.c1 = block(in_ch, h)
        self.c2 = block(h, h * 2)
        # Global mean + max pooling — captures both average and extreme particle properties
        self.head = nn.Sequential(
            nn.Linear(h * 4, h * 2), nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(h * 2, 2)
        )
    ...
    def forward(self, data):
        ...
        x = self.c1(x, ei)
        x = self.c2(x, ei)
        # Concatenate mean and max pool — mean captures typical particle, max captures extremes
...
if args.run:
    ...
    labels = np.array([g.y.item() for g in graphs])
    print(f"  Class balance — real: {(labels==0).sum()}  generated: {(labels==1).sum()}")
    ...
    for n in sorted(ablation_aucs, key=lambda x: full_auc - ablation_aucs[x], reverse=True):
